...
const includeRequestPropertiesInHash = [
  'uri',
  'qs',
  'method',
  'authenticatedUser',
...
const mergeableEvaluatorConfigProperties = [
  'storePrivate',
  'storeNoStore',
  'ignoreNoLastMod',
];
...
function OniyiCache(args) {
  ...
  self.includeRequestPropertiesInHash = _.union(
    ...
    _.isArray(args.includeRequestPropertiesInHash)
      ? args.includeRequestPropertiesInHash
      : [],
  );
  self.excludeRequestHeadersFromHash = _.union(
    excludeRequestHeadersFromHash,
    _.isArray(args.excludeRequestHeadersFromHash)
      ? args.excludeRequestHeadersFromHash
      : [],
  );
  ...
  debug(
    'creating redis client for host "%s" and port "%d"',
    redisOptions.host,
    redisOptions.port,
  );
  self.redisClient = redis.createClient(
  ...
  const data = JSON.stringify(
    _.merge(_.pick(requestObject, self.includeRequestPropertiesInHash), {
      headers: _.omit(
        ...
        self.excludeRequestHeadersFromHash,
      ),
    }),
  );
...
OniyiCache.prototype.clearHostConfigs = function clearHostConfigs(
  hostnamesArray,
) {
...
OniyiCache.prototype.getEvaluator = function getEvaluator(hostname, config) {
  ...
  if (_.isArray(hostConfig.requestValidators)) {
    cacheSettings.requestValidators = hostConfig.requestValidators.concat(
      cacheSettings.requestValidators,
    );
  }
  if (_.isArray(config.requestValidators)) {
    cacheSettings.requestValidators = config.requestValidators.concat(
      cacheSettings.requestValidators,
    );
  }
  ...
  if (_.isArray(hostConfig.responseValidators)) {
    cacheSettings.responseValidators = hostConfig.responseValidators.concat(
      cacheSettings.responseValidators,
    );
  }
  if (_.isArray(config.responseValidators)) {
    cacheSettings.responseValidators = config.responseValidators.concat(
      cacheSettings.responseValidators,
    );
  }
  ...
  self.redisClient.hgetall(key, (getAllError, data) => {
    ...
    if (!data.response) {
      self.redisClient.del(key, (error, result) => {
        ...
        debug(
          'removed %d entries from cache {%s} due to invalid data',
          result,
          hash,
        );
      });
...
OniyiCache.prototype.putWithCommand = function putWithCommand(
  command,
  { hash, data },
  cb,
) {
  ...
  self.redisClient[command](key, data, err => {
    ...
    debug(
      'command %s for hash %s for data %o was successful',
      command,
      hash,
      data,
    );
