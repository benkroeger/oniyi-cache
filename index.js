...
function OniyiCache(args) {
  ...
  if (args.redisClient) {
    ...
    assert(_.isPlainObject(args.redis), 'args.redis must be provided as plain object if no args.redisClient is available');
    var redisOptions = args.redis;
    ...
    if (typeof redisOptions.db === 'number') {
      debug('selecting database "%d"', redisOptions.db);
      self.redisClient.select(redisOptions.db, function (err) {
        if (err) {
          debug('failed to select database {%d}', redisOptions.db);
          self.redisClient.emit('error', err);
        }
      });
    }
  }
