...
const Validators = {
  request: {
    ...
    maxAgeZero: (requestOptions, evaluator) => {
      if (
        (requestOptions.headers['cache-control'] || '').match(maxAgeZeroRegex)
      ) {
  ...
  response: {
    ...
    lastModified: (response, evaluator) => {
      if (
        !_.isUndefined(response.headers['last-modified']) &&
        !evaluator.ignoreNoLastMod
      ) {
        evaluator.flagStorable(true);
        return true;
      }
    ...
    eTag: (response, evaluator) => {
      if (!_.isUndefined(response.headers.etag)) {
        evaluator.flagStorable(true);
        return true;
      }
